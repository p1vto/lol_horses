...
impl ParticipantStats {
    pub fn get_match_score(&self) -> usize {
        let mut score = 100;
        if self.firstBloodKill { score += 10 }
        if self.firstBloodAssist { score += 5 }
        if self.causedEarlySurrender { score -= 10 }
        if self.win { score += 5 } else { score -= 5 }
...
impl GameMatchList {
    ...
    pub fn get_recently_rank_average_score(&self) -> f32 {
        let scores: Vec<usize> = self.games
        .games
        .iter()
        .filter(|game| game.gameMode == "CLASSIC" && game.gameType == "MATCHED_GAME" && game.queueId == 420)
